...
class Translator extends AbstractTranslator
{
    ...
    public function getCatalogue(?string $locale = null): MessageCatalogueInterface
    {
        ...
        if ($this->isCicadaLocaleCatalogue($catalog) && !$this->isFallbackLocaleCatalogue($catalog, $localization)) {
            ...
            // or locale is symfony style locale so we shouldn't add cicada fallbacks as it may lead to circular references
            $fallbackLocale = null;
        }
    ...
    public function trans(string $id, array $parameters = [], ?string $domain = null, ?string $locale = null): string
    {
        ...
        if (!Feature::isActive('cache_rework')) {
            if ($this->fineGrainedCache) {
                ...
                foreach (array_keys($this->keys) as $trace) {
                    $this->traces[$trace]['cicada.translator'] = true;
                }
            }
        }
    ...
    /**
     * Cicada uses dashes in all locales
     * if the catalogue does not contain any dashes it means it is a symfony fallback catalogue
     * in that case we should not add the cicada fallback catalogue as it would result in circular references
     */
    private function isCicadaLocaleCatalogue(MessageCatalogueInterface $catalog): bool
    {
    ...
    private function loadSnippets(MessageCatalogueInterface $catalog, string $snippetSetId, ?string $fallbackLocale): array
    {
        ...
        return $this->cache->get($key, function (ItemInterface $item) use ($catalog, $snippetSetId, $fallbackLocale) {
            if (Feature::isActive('cache_rework')) {
                ...
                $item->tag(\sprintf('translation.catalog.%s', $this->channelId ?: 'DEFAULT'));
            }
