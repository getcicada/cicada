...
abstract class Plugin extends Bundle
{
     ...
     * If needed, return true and Cicada will execute `composer require` during the plugin installation.
     * When the plugins gets uninstalled, Cicada executes `composer remove`
     */
    ...
    {
        // namespace should not start with `cicada`
        if (str_starts_with(mb_strtolower($this->getMigrationNamespace()), 'cicada') && !str_starts_with(mb_strtolower($this->getMigrationNamespace()), 'cicada\commercial')) {
            throw PluginException::cannotDeleteCicadaMigrations();
        }
