...
namespace Cicada\Core\Maintenance;
...
class MaintenanceException extends HttpException
{
    ...
    public static function dbVersionMismatch(
        ...
        return new DatabaseSetupException(
            Response::HTTP_INTERNAL_SERVER_ERROR,
            self::MAINTENANCE_DB_VERSION_SELECT_FAILED,
            'Your database server is running {{ dbKind }} {{ actualVersion }}, but Cicada 6 requires at least MySQL {{ mysqlRequiredVersion }} OR MariaDB {{ mariaDBRequiredVersion }}',
            [
                'dbKind' => $dbKind,
