...
class SnippetService
{
    ...
    public function getFrontendSnippets(MessageCatalogueInterface $catalog, string $snippetSetId, ?string $fallbackLocale = null, ?string $channelId = null): array
    {
        ...
        $usingThemes = $this->getUsedThemes($channelId);
        ...
        // at least overwrite the snippets with the database member overwrites
    ...
    {
        if (!$this->container->has(FrontendPluginRegistry::class)) {
            return [];
        }

        $unusedThemes = $this->container->get(FrontendPluginRegistry::class)->getConfigurations()->getThemes()
            ->filter(fn (FrontendPluginConfiguration $theme) => !\in_array($theme->getTechnicalName(), $usingThemes, true))
            ->map(fn (FrontendPluginConfiguration $theme) => $theme->getTechnicalName());
    ...
    private function getUsedThemes(?string $channelId = null): array
    {
        ...
        if (!$channelId || $this->channelThemeLoader === null) {
