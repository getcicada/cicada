...
class TestBootstrapper
{
    ...
    public function addCallingPlugin(?string $pathToComposerJson = null): TestBootstrapper
    {
        ...
        $baseClass = $composer['extra']['cicada-plugin-class'] ?? '';
        if ($baseClass === '') {
            throw new \RuntimeException('composer.json does not contain `extra.cicada-plugin-class`. Path: ' . $pathToComposerJson);
        }
