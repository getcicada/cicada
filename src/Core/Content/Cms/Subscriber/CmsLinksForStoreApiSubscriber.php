...
use Cicada\Core\Framework\Api\ApiDefinition\DefinitionService as ApiDefinition;
...
class CmsLinksForStoreApiSubscriber implements EventSubscriberInterface
{
    ...
    public function relativeLinksForStoreAPIOutput(
        ...
        if ($this->isStoreApiRequest($request)) {
            ...
            $this->processCmsPageCollection($pages, $request->getSchemeAndHttpHost(), $event->getChannelContext());
        }
    }
    ...
    private function processCmsPageCollection(
        ...
        $transformedChunks = $this->processAndSplitContent($concatenatedContent, $schemeAndHttpHost, $channelContext);
    ...
    private function processAndSplitContent(
        ...
        $concatenatedContentReplaced = $this->replaceLinks($concatenatedContent, $schemeAndHttpHost, $channelContext);
        if ($this->delimiter === '') {
            return [];
        }
    ...
    private function replaceLinks(string $content, string $schemeAndHttpHost, ChannelContext $channelContext): string
    {
        $content = $this->mediaUrlReplacer->replace($content);
        $content = $this->seoUrlReplacer->replace($content, '', $channelContext);
